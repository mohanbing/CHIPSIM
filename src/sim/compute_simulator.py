...
class ComputeSimulator:
    """
    Handles compute simulation using CimLoop API with caching.
    """
    def __init__(self, 
                 cache_file="compute_cache.json", 
                 clear_cache=False):
        """
        ...
        # Initialize the CIMLoop API instance
    ...
    def simulate_compute(self, yaml_config, chiplet_id, chiplet_type, batch_size=1):
        """
        Simulate the compute of a single model chunk using its YAML config.
        ...
        """
        # Track the time for the API call
        api_start_time = time.time()
        
        # First, check if this simulation is cached
        ...
        if self.cache_manager.has_result(cache_key):
            print(f"🔄 Using cached simulation result for chiplet {chiplet_id}")
            return self.cache_manager.get_result(cache_key)
        
        # If not cached, run the simulation
        try:
            print(f"🔄 Making API request for chiplet {chiplet_id}...")
            ...
            print(f"⏱️  API call completed in {api_duration:.3f} seconds")
            ...
            error_msg = f"Exception during CimLoop API call: {str(e)}"
            print(f"❌ {error_msg}")
            raise
        ...
        print(f"✅ Simulation results: Latency={latency_us:,.2f} μs, Energy={energy_fj:,.2f} fJ")
